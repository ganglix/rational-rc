...
def membrane_failure_year(model, year_lis, plot=True, amplify=30):
    """
    Run the membrane model over a list of time steps and optionally plot results.

    Parameters:
    -----------
    model : class instance
        MembraneModel class instance.
    year_lis : list
        Time steps (years).
    plot : bool
    ...
    if plot:
        fig, [ax1, ax2, ax3] = plt.subplots(
            nrows=3,
            figsize=(8, 8),  # original size retained
            sharex=True,
        ...
        ax1.plot([m.t for m in M_lis], pf_vals, "k--")
        ...
        for m in M_sel:
            plot_RS_special(m, ax=ax3, t_offset=m.t, amplify=amplify)

        ax3.set_xlabel("Time [year]", fontsize=16)
        ax3.set_ylabel("Age / Membrane Life [year]", fontsize=16)
        ax3.legend(
            handles=[
                mpatches.Patch(color="C0", label="R: membrane life", alpha=0.8),
                mpatches.Patch(color="C1", label="S: age", alpha=0.8),
            ],
            loc="upper left",
            fontsize=16
        )
