...
class TestHelperFunc(unittest.TestCase):
    ...
    def test_interp_extrap_f(self):
        ...
        res1 = hf.interp_extrap_f(x1, y1, xfind, plot=False)
        np.testing.assert_almost_equal(res1, np.array([1., 1.4]))

        x2 = np.linspace(0, 1, 3)
        y2 = np.linspace(0, 2, 3)
        res2 = hf.interp_extrap_f(x2, y2, xfind, plot=False)
        np.testing.assert_almost_equal(res2, np.array([1, 1.4]))

    def test_find_similar_group(self):
        lis = [1, 1.1, 2, 3]
        self.assertCountEqual(hf.find_similar_group(lis, similar_group_size=2), [1, 1.1])

    def test_sample_integral(self):
        Y35 = np.array([[1, 1.1, 1.01, 1.001, 1.0001],
                        [2, 2.1, 2.01, 2.001, 2.0001],
                        [3, 3.1, 3.01, 3.001, 3.0001]])
        x5 = np.array([1, 2, 3, 4, 5])
        with self.assertRaises(Exception):
            hf.sample_integral(Y35, x5)

        x2 = np.array([1, 2, 3])
        np.testing.assert_almost_equal(hf.sample_integral(Y35, x2), np.array([4., 4.2, 4.02, 4.002, 4.0002]))

    def test_f_solve_poly2(self):
        self.assertTrue(1.0 in hf.f_solve_poly2(1, -2, 1))
    ...
    def test_Pf_RS(self):
        ...
        pf_RS, beta_factor, R_distrib, S_kde_fit = hf.Pf_RS(R_info=(2, 0.1), S=sample, R_distrib_type='normal', plot=False)
        ...
        self.assertAlmostEqual(S_kde_fit.resample(100000).mean(), sample.mean(), places=2)

        pf_RS, beta_factor, R_distrib, S_kde_fit = hf.Pf_RS(R_info=(2, 0.5, 0, 8), S=sample, R_distrib_type='beta', plot=False)
    ...
    def test_find_mean(self):
        std = 10 / 1.64
        cutoff = 30
        self.assertAlmostEqual(hf.find_mean(cutoff, std, confidence_one_tailed=0.95), 40.029595286288796, msg='95% one tailed')
        self.assertAlmostEqual(hf.find_mean(cutoff, std, confidence_one_tailed=0.90), 37.81433881429602, msg='90% one tailed')
