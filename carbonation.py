...
def R_ACC_0_inv(pars):
    ...
    if (isinstance(x_c, int) or isinstance(x_c, float)):
        ...
        R_ACC_0_inv_temp = hf.Normal_custom(R_ACC_0_inv_mean, R_ACC_0_inv_stdev)  # [(m^2/s)/(kg/m^3)]
        ...
        y = fit_df.values
        R_ACC_0_inv_mean = hf.interp_extrap_f(x, y, pars.option.wc_eqv, plot=False) * \
            1e-11  # [(m^2/s)/(kg/m^3)] #interp_extrap_f: defined function
...
def calibrate_f(model_raw, t, carb_depth_field, tol=1e-6, max_count=50, print_out=True):
    ...
    if print_out:
        print('carb_depth:')
        print('model: \nmean:{}\nstd:{}'.format(hf.Get_mean(model.xc_t), hf.Get_std(model.xc_t)))
...
class Carbonation_Model:
    ...
    def postproc(self, plot=False):
        sol = hf.Pf_RS((self.pars.cover_mean, self.pars.cover_std), self.xc_t, plot=plot)
        self.pf = sol[0]
